...
> **Android needs intempt-android 5.0.0 for that.** This package bundles 4.0.1 by default
> (`intemptAndroidVersion` in `android/build.gradle`), and 4.x starts autocapture at `init()`.
> Set `intemptAndroidVersion=5.0.0` in your app's `gradle.properties` until a release of
> this package bundles it.
...
On Android that also needs intempt-android 5.0.0. To send a typed value on purpose, pass
